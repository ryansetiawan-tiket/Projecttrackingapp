...
export function ProjectTableRow({
  ...
  return (
    ...
    >
      ...
      <TableCell className={`${config.rowPadding} w-[420px] min-w-[420px] max-w-[420px]`}>
        <div className="space-y-2 mx-[2px] my-[0px] p-[0px]">
          <div className="flex items-center gap-2">
            ...
            {/* Notes Icon with Tooltip - Only show if notes exist */}
      ...
      {/* Status Cell */}
      <TableCell className="w-[140px] min-w-[140px] max-w-[140px] text-center" onClick={(e) => e.stopPropagation()}>
        {isPublicView ? (
          <Badge 
            variant="outline" 
            className="text-xs"
                  ...
                  onClick={(e) => {
                    ...
                    if (status === 'Done' && project.actionable_items && project.actionable_items.length > 0) {
                      ...
                      handlers.onUpdate(project.id, { 
                        ...
                        completed_at: new Date().toISOString() // Track completion timestamp
                      });
                    } else if (status === 'Done') {
                      // Status changed to Done without assets
                      handlers.onUpdate(project.id, { 
                        status,
                        completed_at: new Date().toISOString() // Track completion timestamp
                      });
                      ...
                      handlers.onUpdate(project.id, { 
                        status,
                        completed_at: null // Clear completion timestamp when not Done
                      });
                    }
                  }}
      ...
      {/* Start Date Cell */}
      <TableCell className="w-[120px] min-w-[120px] max-w-[120px] text-center" onClick={(e) => e.stopPropagation()}>
        <DateCell
      ...
      {/* Actions Cell - Only show if not public view */}
      {!isPublicView && (
